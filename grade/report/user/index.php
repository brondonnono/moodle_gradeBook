...
if (has_capability('moodle/grade:viewall', $context)) { //Teachers will see all student reports
    ...
    if ($report->fill_table()) {
        ...
        echo '<script> function print(){';
        echo 'let windowContent = `<!DOCTYPE html><html><head></head><body>${}</body></html>`; ';
        echo 'let WindowObject = window.open("", "PrintWindow", "width=750,height=650,top=50,left=50,toolbars=no,scrollbars=yes,status=no,resizable=yes");';
        echo 'WindowObject!.document.writeln(windowContent);';
        echo 'WindowObject!.document.close(); WindowObject!.focus();';
        echo 'WindowObject!.onload = function () { WindowObject!.print(); WindowObject!.close(); }';
        echo'}</script>';
        echo '</div>';
        echo "<input type='submit' id='printBtn' class='btn btn-primary' value='Impimer' onclick='print()'>";
    }
}
